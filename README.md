...
Для *epf*/*erf* модули лежат в корне каталога распаковки (`0_`/`1_`/`2_` + `_bin`/`_meta`).
Для *cf*/*cfe* — корень `_bin`/`_meta`/`_objects` плюс модули конфигурации (`0_`/`1_`/`2_`/`9_`);
объекты метаданных — в `_objects/Catalogs|Documents|…/Имя/` (английские имена классов 1С).
